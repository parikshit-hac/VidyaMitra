...
function ResumeUpload() {
  ...
  const summaryLines = useMemo(() => {
    const raw = String(resumeAnalysis?.ai_summary || "");
    return raw
      .split(/\n+/)
  ...
  const roadmapLines = useMemo(() => {
    const raw = careerResult?.langchain_roadmap || "";
    return raw
      .split("\n")
      .map((line) => line.replace(/^[-*•\d.)\s]+/, "").trim())
      .filter(Boolean);
  ...
  return (
    <section className="space-y-5">
      ...
      {careerResult && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900">Career Support Guidance</h3>
            <p className="mt-2 whitespace-pre-wrap text-sm text-slate-700">
              {careerResult.ai_guidance}
            </p>
          ...
          <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900">Roadmap Suggestions</h3>
            {roadmapLines.length > 0 ? (
              <ul className="mt-2 space-y-2 text-sm text-slate-700">
                {roadmapLines.map((line, idx) => (
                  <li key={`${line}-${idx}`} className="rounded-md bg-slate-50 px-3 py-2">
                    {line}
                  </li>
                ))}
              </ul>
            ) : (
              <p className="mt-2 text-sm text-slate-700">{careerResult.langchain_roadmap || "No roadmap returned."}</p>
            )}
            <p className="mt-3 text-xs text-slate-500">Source: {careerResult.agent_source || "unknown"}</p>
          </article>
        </div>
      )}
    </section>
